<?php
//constant definition
define('MYHOST','localhost');
define ('MYROOT','root');//schsearch_user
define ('PASSWORD','');//Pa$$w0rd@1234
define ('MYDATABASE','schoolsearchdb');//schoolsearchdb
define ('BIN','lsa_bin');
define ('FEES','lsa_fees');
define ('LOGS','lsa_main_user_tbl');
define ('PAYTBL', 'payment_transaction');
define ('NEWS', 'lsa_news_notification_tble');
define ('ROOT',$_SERVER['HTTP_HOST']);

define("TESTKEY", "sk_test_a0b12fef17fc46e2cbed1507223421b45c71245d");
define("LIVEKEY", "");
?>